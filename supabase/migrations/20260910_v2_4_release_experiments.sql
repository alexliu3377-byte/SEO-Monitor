...
insert into public.development_releases (
  ...
  '["反馈入口改为按钮，留言板使用整齐的列表、筛选和未读优先排序", "反馈详情改为聊天式沟通，并保留完整问题、相关页面和站点资料", "新增站点提交，组员和管理员可提交候选站点供超管审核", "项目负责人可更新处理进度并删除完全无关的反馈", "开发日志以版本号为重点，统一展开交接说明并改善编辑弹窗", "内部备注改为所有超管可读、仅项目负责人可编辑", "登录后先进入后台选择首页，内容发布系统与应用更新系统使用独立路径和导航"]'::jsonb,
  '["趋势发现：小红书与抖音低频采集、候选词评分及采集词设置，目前继续观察数据价值和账号稳定性。", "应用更新中心：登记公开更新来源、识别版本与更新日志、人工审核和批量导出，目前继续验证不同站点的识别准确率。"]'::jsonb,
  '["反馈列表由服务端分页、筛选并计算留言数量与每位用户的未读状态。", "普通反馈采用公开留言板范围，回复权限仍限制为提交者与项目负责人；超管重点沟通保持独立范围。", "登录后的根路径作为系统选择页，内容后台统一使用 /content 路径；旧内容地址会自动跳转，保留原有收藏和入口。", "实验工作区继续使用现有登录体系，但页面、API 和数据库访问同时检查超管权限。", "趋势采集由已登录的本地浏览器低频执行；应用更新页面读取由独立 GitHub Actions 手动任务执行。"]'::jsonb,
