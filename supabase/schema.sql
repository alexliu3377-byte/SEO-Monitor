...
-- 这个词在所有站点里最高的连续天数，"查看"时展开全部站点各自的连续天数
-- （sites jsonb 数组，一开始封顶前5名，用户反馈直接要求不设上限，改成全展示）。
...
LANGUAGE sql STABLE AS $$
  ...
  SELECT
    ...
    JSONB_AGG(JSONB_BUILD_OBJECT('domain', domain, 'streak', streak, 'volume', volume, 'dates', dates) ORDER BY streak DESC, volume DESC) AS sites
  FROM per_site
