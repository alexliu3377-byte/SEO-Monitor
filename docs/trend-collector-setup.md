...
第一阶段使用个人 Windows 电脑和独立的浏览器资料目录，低频读取小红书、抖音搜索结果首屏。采集器不会读取或上传账号密码、Cookie、图片、视频和完整评论；来源链接会先移除查询参数和锚点；遇到登录或验证要求会停止。
...
主题和频率相关设置位于 `config/trend-collector.json`。第一阶段每个平台只有 8 个宽泛主题，每个主题最多读取 12 条首屏结果。
