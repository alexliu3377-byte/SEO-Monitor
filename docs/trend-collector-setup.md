...
当前使用独立私有仓库 `trend-collector-worker` 调度专用 Windows 电脑上的 self-hosted Runner，低频读取小红书、抖音搜索结果首屏。主仓库只提供任务领取和结果接收 API，不执行平台浏览器采集。采集器不会上传账号密码、Cookie、图片、视频和完整评论；遇到登录或验证要求会停止。
...
每个主题最多读取 12 条首屏结果。后台每个平台最多维护100个入口词；独立 Worker 每轮只领取一个到期词，同词至少间隔12小时，词量增加时自动延长轮换周期。

搜索什么词不需要再修改配置文件。项目负责人打开网站的“趋势发现”，点击右上角“设置采集词”，可以分别维护小红书和抖音的搜索入口词。采集器每次运行前会自动读取网站上保存的最新设置，并在当轮临时打乱平台和采集词的执行顺序；网站暂时无法连接时，才会使用 `config/trend-collector.json` 中的备用词。
