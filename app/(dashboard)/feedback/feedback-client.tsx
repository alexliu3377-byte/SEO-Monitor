        ...
        <Modal title={priorityDiscussion ? '超管沟通详情' : '反馈留言详情'} onClose={closeDiscussion} width="max-w-4xl">
          <div className="space-y-4">
            <section className="rounded-xl border border-slate-200 bg-white p-5">
              <h2 className="text-xl font-bold leading-8 text-slate-950">{discussionItem.title}</h2>
              <div className="mt-2 flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-slate-500"><span>{TYPE[discussionItem.feedback_type].label}</span>{discussionItem.related_page && <span>相关页面：{PAGE[discussionItem.related_page] ?? discussionItem.related_page}</span>}<span>{discussionItem.created_by_name} · {ROLE[discussionItem.submitter_role].label}</span><span>提交于 {dateTime(discussionItem.created_at)}</span><span className="font-medium text-slate-700">{STATUS[discussionItem.status].label}</span></div>
              <p className="mt-4 whitespace-pre-wrap text-sm leading-7 text-slate-700">{discussionItem.details}</p>
              {discussionItem.submitted_site && <a href={`https://${discussionItem.submitted_site}`} target="_blank" rel="noreferrer" className="mt-3 inline-flex font-mono text-sm font-medium text-emerald-700 hover:underline">{discussionItem.submitted_site} ↗</a>}
              {discussionItem.owner_response && <div className="mt-4 border-l-2 border-emerald-500 pl-3 text-sm leading-6 text-slate-700"><span className="font-semibold text-slate-900">负责人回复：</span>{discussionItem.owner_response}</div>}
              {discussionItem.problem_details && <div className="mt-3 border-l-2 border-red-400 pl-3 text-sm leading-6 text-slate-700"><span className="font-semibold text-slate-900">问题详情：</span>{discussionItem.problem_details}</div>}
            </section>
            <div className="flex items-center justify-between gap-3"><h3 className="font-semibold text-slate-900">{priorityDiscussion ? '沟通记录' : '留言记录'} <span className="font-normal text-slate-400">({messageTotal})</span></h3><p className="text-xs text-slate-400">项目负责人在左 · 反馈人员在右</p></div>
            ...
            {!messageLoading && !messageError && <div className="min-h-36 rounded-xl bg-slate-200/60 p-4 sm:p-5">{messages.length === 0 ? <p className="py-10 text-center text-sm text-slate-500">还没有留言记录。</p> : <div className="space-y-4">{messages.map(item => { const ownerMessage = item.is_project_owner; const messageMeta = MESSAGE_TYPE[item.message_type]; return <div key={item.id} className={`flex ${ownerMessage ? 'justify-start' : 'justify-end'}`}><article className={`max-w-[82%] rounded-2xl px-4 py-3 shadow-sm sm:max-w-[72%] ${ownerMessage ? 'rounded-tl-md border border-slate-200 bg-white' : 'rounded-tr-md bg-emerald-100'}`}><div className="flex flex-wrap items-center gap-2 text-xs"><span className="font-semibold text-slate-800">{ownerMessage ? '项目负责人' : priorityDiscussion ? '超管反馈人员' : '反馈人员'} · {item.author_name}</span>{priorityDiscussion && <span className="text-slate-500">{messageMeta.label}</span>}</div><p className="mt-1.5 whitespace-pre-wrap text-sm leading-6 text-slate-800">{item.content}</p><p className="mt-1.5 text-right text-[11px] text-slate-400">{dateTime(item.created_at)}</p></article></div>})}</div>}</div>}
