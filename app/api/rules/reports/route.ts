...
export async function GET() {
  ...
  const { data: reports, error } = await service
    ...
    .select('id, period_start, period_end, status, sites_considered, sites_analyzed, sites_skipped, gemini_call_count, gemini_fail_count, error, created_at, completed_at')
