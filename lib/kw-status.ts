...
// 2026-08-17 当天再收紧一档：用户反馈偏高/偏低的触发比例还是想再少一些。
// 2.5→4，用最近7天真实数据验证过：触发率从13.1%（41/314）降到7.0%
// （22/314），且已知的真实异常（3h3.com连续多天新增归零）在新阈值下第一天
// 依然能正常触发，只是不再重复报后续几天——不是漏检，是判定门槛本身变严了。
