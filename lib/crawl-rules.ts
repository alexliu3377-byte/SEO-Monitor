...
export const CRAWL_RULES: RuleSection[] = [
  ...
  },
  {
    key: 'research-report',
    title: '研究报告（周/月/年）',
    badge: 'research-report.yml · GitHub Actions · 周一07:30/每月1号08:00/每年1月1号08:30 MYT',
    items: [
      { label: '触发方式', text: 'GitHub Actions research-report.yml，同一个workflow三条cron，2026-08-10起彼此错开（此前三条挤在同一个时间点，1月1号那天月报年报会同时触发；周报当时晚点1.5小时才跑，顺带把整点排队延迟这条也一起考虑进去）：周报（cron 30 23 * * 0 UTC=每周一07:30 MYT，UTC按周日23:30算是因为跨天）、月报（cron 0 0 1 * * UTC=每月1号08:00 MYT，覆盖上个月，用户自己选的整点，接受排队延迟风险）、年报（cron 30 0 1 1 * UTC=每年1月1号08:30 MYT，覆盖上一年）；用 github.event.schedule 判断这次是哪种周期。也可 workflow_dispatch 手动指定 periodType(week/month/year)+periodStart/periodEnd' },
      ...
      { label: 'Stage 1（逐站点，分片内串行）', text: '每个分片内部对分到的 is_enabled=true 站点，用 lib/site-research-summary.ts 的 fetchSiteResearchSummary() 拉这段周期的权重/收录/涨跌/新增关键词/排名成效全量明细（不压缩，月报/年报同样是完整重新读原始数据，不是把周报文字汇总）。2026-08-10 起同时用这份明细代码算出结构化数字（最新一天的 pc_weight/mobile_weight、期间平均收录 avg_index_count、期间平均移动IP avg_mobile_ip），直接存进 research_report_sites 对应4个新列，不经过AI；再用 lib/site-research-prompt.ts 的 buildSiteAnalysisPrompt() 建 prompt；单次 callGeminiJSON 调用（maxOutputTokens 4096）；这段时间完全没有任何抓取数据的站点直接跳过，不占AI调用（也不写4个结构化数字列）；每次调用间隔4秒；单站点数据拉取失败或AI调用失败都不中断这个分片，记录失败原因后跳到下一个站点；每个站点跑完立刻 upsert 进 research_report_sites（每站点一行，按 report_id+site_id 唯一——分片并行写不会互相覆盖，这是跟旧版单进程共享一个 JSONB 数组列最大的区别）' },
