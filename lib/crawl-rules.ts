...
export const CRAWL_RULES: RuleSection[] = [
  ...
  },
  {
    key: 'research-report',
    title: '研究报告（周/月/年）',
    badge: 'research-report.yml · GitHub Actions · 周一/每月1号/每年1月1号 08:10 MYT',
    items: [
      { label: '触发方式', text: 'GitHub Actions research-report.yml，同一个workflow三条cron：周报（cron 10 0 * * 1 UTC=每周一08:10 MYT）、月报（cron 10 0 1 * * UTC=每月1号08:10 MYT，覆盖上个月）、年报（cron 10 0 1 1 * UTC=每年1月1号08:10 MYT，覆盖上一年；1月1号这天月报年报都会跑）；分钟故意不设整点（10分），整点是GitHub schedule排队高峰容易延迟/漏跑，2026-08-10真实遇到过第一次该触发的周报晚了近1.5小时；用 github.event.schedule 判断这次是哪种周期。也可 workflow_dispatch 手动指定 periodType(week/month/year)+periodStart/periodEnd' },
