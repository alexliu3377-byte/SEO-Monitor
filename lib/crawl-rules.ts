...
export const CRAWL_RULES: RuleSection[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { label: '并行策略', text: '排名段1-5各开一个浏览器 page，起跑前各自随机等待0-2秒错峰（避免5个page同时发第一个请求，看起来太像脚本）；段内按页顺序，每页间隔随机3-5秒（2026-07-27 起从固定300ms调整，固定短间隔+5段完全同步起跑被怀疑是生产环境部分站点被打回挑战页、涨跌其中一侧结果偏少的原因之一）' },
  ...
  },
  {
    ...
    items: [
      ...
      { label: '并行策略', text: '排名段 1-5 各开一个浏览器 page，起跑前各自随机等待0-2秒错峰；段内按页顺序，每页间隔随机3-5秒（与 rank 步骤相同，详见 rank 小节"并行策略"）；4 个组合顺序执行，组合间隔随机 3-5 秒；站点间间隔 60 秒' },
