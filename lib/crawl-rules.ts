...
export const CRAWL_RULES: RuleSection[] = [
  ...
  {
    key: 'weekly-research-report',
    title: '每周研究报告',
    badge: 'weekly-research-report.yml · GitHub Actions · 每周一 08:00 MYT（cron 00:00 UTC）',
    items: [
      { label: '触发方式', text: 'GitHub Actions weekly-research-report.yml（cron 0 0 * * 1 UTC = 08:00 MYT 每周一），晚于 daily-crawl 最后一步 tracking（06:45 MYT）和 environment-snapshot（07:15 MYT）；也可 workflow_dispatch 手动指定 periodStart/periodEnd（留空则默认取上周一~上周日）；脚本：scripts/research-report.ts（GitHub Actions runner 直接跑，不经过 Vercel——单次运行约80+次 Gemini 调用，远超 Vercel maxDuration 上限）' },
      { label: 'Stage 1（逐站点）', text: '对每个 is_enabled=true 站点，用 lib/site-research-summary.ts 的 fetchSiteResearchSummary() 拉这一周的权重/收录/涨跌/新增关键词/排名成效全量明细（不压缩），用 lib/site-research-prompt.ts 的 buildSiteAnalysisPrompt() 建 prompt（要求AI判断权重上涨是否连续多天而非单日跳动），单次 callGeminiJSON 调用；这一周完全没有任何抓取数据的站点直接跳过，不占AI调用；每次调用间隔4秒；单站点AI调用失败不中断整体运行，记录失败原因后跳到下一个站点；每个站点跑完立刻写回 weekly_research_reports.site_analyses（断点续跑：同周期重跑会跳过已经有结果的站点）' },
      { label: '自己团队成效', text: '各 task_groups 用 lib/tracking-summary.ts 的 fetchGroupEffectivenessSummary()（复用与"追踪汇总"相同的 dedupe/得分逻辑）取这一周排名/收录成效汇总 + 得分最高的15条，不单独占AI调用，原始数据直接留到 Stage 2 一起喂' },
      { label: '大环境输入', text: '查这一周 environment_daily（大盘每日）+ environment_segments_daily（体量档位/内容侧重两个维度的分段数据）全部行，存进 weekly_research_reports.environment_input 做历史快照（不依赖以后这两张表的算法是否变化）' },
      { label: 'Stage 2（一次综合）', text: '把大环境数据（格式化文本）+ 全部 Stage1 站点分析文字（AI写好的分析，不是原始数字）+ 自己团队原始成效数据一次性喂给 Gemini，maxOutputTokens=8192，要求返回 {environment, effectiveness, conclusion} 三段 JSON' },
      { label: '写入表', text: 'weekly_research_reports（按 period_start+period_end 唯一 upsert；site_analyses/own_team_analyses/environment_input/report_sections 均为 jsonb；永久保留）' },
      { label: '不写入', text: '不写 rules 表——旧版"AI分析后可转成规则"的流程已随本次重构下线，报告是纯叙事性文字，不产出结构化规则' },
    ],
  },
  {
