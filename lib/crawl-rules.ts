...
export const CRAWL_RULES: RuleSection[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { label: '采集范围', text: '读取超管登记的公开更新页面，第一阶段只提取版本号、更新日志、发布日期、包大小、公开下载链接及来源页面；不登录第三方后台、不绕过验证、不下载或保存 APK/安装包文件' },
      { label: '访问限制', text: '每个来源顺序执行并至少间隔4秒；单页最长等待30秒、响应正文上限5MB、最多跟随3次重定向。每次请求和每一跳重定向都检查 DNS 与目标地址，拒绝本机、私网、保留地址和非标准端口，避免服务端请求伪造' },
      { label: '自动识别', text: '优先读取来源配置的 CSS 选择器，其次读取网页 meta、JSON-LD、常见“更新日志/版本说明”标题和保守的版本号规则。页面结构不统一时可能识别不到或识别错误，因此新记录先进入“待审核”，不会直接成为应用的正式最新版本' },
      ...
      { label: '实验边界', text: '当前是 v4.0.0 的线上实验区，不写入正式开发版本记录。先验证哪些公开页面稳定、自动识别准确率和导出格式，再决定是否加入定时任务、站点专用解析器、批量应用导入或安装包处理' },
    ],
  },
  {
    key: 'search',
