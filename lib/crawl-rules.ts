...
export const CRAWL_RULES: RuleSection[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { label: '限流与失败', text: '平台与查询词顺序执行；小红书每轮8个主题、抖音每轮4个主题，每个查询最多12条结果，查询之间至少等待8秒。2026-09-09 首轮实测抖音连续到第8个主题时出现访问频繁，因此主动减半。出现登录、验证码、安全验证或访问频繁时立即停止该平台并记录 blocked，不自动绕过验证、不做高频重试、不使用代理池' },
