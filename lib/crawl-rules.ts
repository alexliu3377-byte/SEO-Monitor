...
export const CRAWL_RULES: RuleSection[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { label: '竞品成效', text: '用 lib/competitor-effectiveness.ts 的 fetchCompetitorEffectivenessSummary() 读 competitor_tracking_records（has_rank_title=true 的竞品站点每天自动被 scripts/crawl.ts 的 runTracking() 追踪），取这段周期的 有效/追踪中/无效 计数 + 得分最高的15条，不单独占AI调用，原始数据直接留到 Stage 2 一起喂，在 finalize job 里算' },
      ...
      { label: 'Stage 2（一次综合，finalize job）', text: '汇总 research_report_sites 里这份报告的全部站点结果（不分片，一次性查全部）+ 大环境数据（格式化文本）+ 竞品成效原始数据一次性喂给 Gemini，maxOutputTokens=8192，要求返回 {environment, effectiveness, conclusion} 三段 JSON；Stage2 本身崩溃会把报告标记 status=failed（不会像单个分片崩溃那样只是不影响别的分片，因为它是最后一段，没有后续步骤能兜底）' },
      { label: '写入表', text: 'research_reports（按 period_type+period_start+period_end 唯一 upsert；competitor_effectiveness/environment_input/report_sections 均为 jsonb；旧版遗留的 site_analyses 列不再写入但保留不删，读接口对拆分片之前生成的老报告仍会读这一列做兼容）/ research_report_sites（每站点一行，按 report_id+site_id 唯一，2026-08-07 新增，随 research_reports 级联删除）' },
  ...
  },
  {
    ...
    badge: '纯展示，不新增抓取逻辑',
    items: [
      { label: '数据来源', text: '/api/research/competitor-sites 和 [id]/tracking 两个只读接口，直接查 competitor_tracking_records（写入逻辑见上方"成效追踪"小节，这里不重复实现）——研究中心"竞品成效"tab 是这张表第一次有UI展示' },
      { label: '添加竞品站点', text: '研究中心"添加竞品站点"按钮复用 components/add-site-modal.tsx（跟网站管理新增站点同一个表单），创建成功后额外调一次 PUT /api/sites 把新站点的 has_rank_title 设成 true，使其立刻进入每日竞品追踪范围，不需要用户再去网站管理里手动开关' },
    ],
  },
  {
