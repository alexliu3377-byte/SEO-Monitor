...
export const CRAWL_RULES: RuleSection[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { label: '提交查询分页（2026-08-26 修复）', text: '"自己站点追踪"这一步要先查全站90天内 status=submitted 的提交（scripts/crawl.ts 和 app/api/cron/route.ts 各一份），之前没分页也没排序，全站量涨到5000+条后撞上 Supabase 单次查询硬顶3000行，且没有 ORDER BY 时被截断的偏偏是最近提交的那部分——导致近期提交大比例（实测92%）从未写入 site_tracking_records，成效追踪里彻底看不到，不是UI筛选问题。已改成真分页（fetchAllRows）+ 按 id 排序兜底' },
  ...
  },
  {
    key: 'research-report',
    title: '研究报告（周/月/季/年）',
    badge: 'research-report.yml · GitHub Actions · 周一07:30/每月1号08:00/1·4·7·10月1号09:00/每年1月1号09:45 MYT',
    items: [
      { label: '触发方式', text: 'GitHub Actions research-report.yml，同一个workflow四条cron，2026-08-20新增季报后彼此错开（季报/年报都要等当天的月报先跑完才有东西可汇总，往后挪留缓冲；季报和年报互相独立、都只依赖月报，不用互相等待，见下方"逐层汇总"）：周报（cron 30 23 * * 0 UTC=每周一07:30 MYT，UTC按周日23:30算是因为跨天）、月报（cron 0 0 1 * * UTC=每月1号08:00 MYT，覆盖上个月）、季报（cron 0 1 1 1,4,7,10 * UTC=1/4/7/10月1号09:00 MYT，覆盖上一季度）、年报（cron 45 1 1 1 * UTC=每年1月1号09:45 MYT，覆盖上一年，2026-08-20从08:30挪后留缓冲）；用 github.event.schedule 判断这次是哪种周期。也可 workflow_dispatch 手动指定 periodType(week/month/quarter/year)+periodStart/periodEnd' },
      ...
      { label: 'Stage 1——周报读原始数据，月/季/年报改成"逐层汇总"（2026-08-20）', text: '之前月报/年报也是各自完整重读原始数据，跟周报一样——这是"以后想收紧 rank_changes/raw_keywords/site_keyword_ranks 保留期"最大的隐患（年报要读整一年，保留期一旦设短会被静默截断）。改成只有周报继续用 lib/site-research-summary.ts 的 fetchSiteResearchSummary() 拉这段周期的权重/收录/涨跌/新增关键词/排名成效全量明细（不压缩），喂给 lib/site-research-prompt.ts 的 buildSiteAnalysisPrompt()；月报汇总这个月内全部已完成的周报、季报汇总这个季度内全部已完成的月报、年报也汇总这一年内全部已完成的月报（不经过季报，见 scripts/research-report.ts 的 CHILD_TYPE 常量——年报和季报都只依赖月报、互不等待，避免1月1号那天月报→季报→年报串成一条严格顺序链），读的是下一级 research_report_sites 里已经产出的 analysis/momentum_keywords/findings，喂给 buildSiteRollupPrompt() 综合出跨周期的规律；某个子周期这个站点没数据或分析失败就不喂那段，全都没有就整体标记skipped。两条路径最终输出的 JSON schema 完全一样（{summary, momentumKeywords, findings}），Stage 2 和页面展示不用区分是哪种路径产出的。pc_weight/mobile_weight/avg_index_count/avg_mobile_ip 这4个结构化数字，月/季/年报改用新增的轻量函数 fetchSiteTrendStats()（只读 weight_history/index_snapshots，这两张表体量小、不受保留期收紧影响，可以放心整年查）算，不再依赖 fetchSiteResearchSummary()。这段时间完全没有可用数据/分析的站点直接跳过，不占AI调用（也不写4个结构化数字列）；每次调用间隔4秒；单站点失败不中断这个分片；每个站点跑完立刻 upsert 进 research_report_sites（每站点一行，按 report_id+site_id 唯一）' },
      { label: 'Stage 1 输出改版（2026-08-12，从"摘要"改成"研究结果"）', text: '用户把旧版prompt拿去问ChatGPT，核心批评是"只让AI点1-2个点、写100-200字"逼着AI当摘要器——排名词的 prev_rank（较上期排名）、searchVolumeRising 里的 prev_volume/volume_change 这些delta数字，lib/site-research-summary.ts 的 EffectivenessRow 早就算好了，之前的prompt却只把它压成一个"🔥需求涨"布尔标记喂给AI，涨了多少名/多少搜索量这些真正有价值的信息在到达AI之前就丢了。现在把这些数字原样暴露（排名词列表格式改成"第X名/较上期Y名升Z/量A较上期涨B/分C/内容分类"，分类用 lib/content-category.ts 的 classifyContentCategory() 现算），JSON schema 从 {analysis} 改成 {summary, momentumKeywords, findings}——summary 保留给页面短展示（存进 analysis 列，列名不变，向后兼容），momentumKeywords 是AI自己判断出的"本期真正在发力的词"按语义聚类（数量不固定，最多30个，不硬凑），findings 是2-5条带置信度（high/medium/low）的具体发现。新增 research_report_sites.momentum_keywords/findings 两个jsonb列存这两块' },
      ...
      { label: '自己站点成效 vs 竞品成效（2026-08-07 拆开，不再混在一起）', text: '"成效"曾经只有竞品一段，用户反馈要跟自己站点的成效分开看。自己站点成效换回之前用过的机制：lib/tracking-summary.ts 的 fetchGroupEffectivenessSummary()，按 task_groups 逐个组读 site_tracking_records（组员提交内容+URL的排名/收录追踪），取 获取排名/获取收录/追踪中/无效 四分类计数 + 得分最高15条；2026-08-10 起额外对每条"获取排名"的claim按 lib/content-category.ts 的 classifyContentCategory() 归类（游戏/应用/专题/资讯——先按URL路径关键字（/game/、/zt/、/article/等）匹配，匹配不上再退回看排名词本身，都判断不出来默认归应用），算出 contentBreakdown 计数，回答"这个月的收益主要是什么类型内容带来的"。竞品成效：lib/competitor-effectiveness.ts 的 fetchCompetitorEffectivenessSummary() 读 competitor_tracking_records（has_rank_title=true 的站点每天自动被 scripts/crawl.ts 的 runTracking() 追踪），取 有效/追踪中/无效 三分类计数 + 得分最高15条+按 content_type（爬虫写入时已经是game/app，不用再归类）算 contentBreakdown——已排除自己的站点（见下方"自己站点排除"）。两份都不单独占AI调用，原始数据+contentBreakdown 直接留到 Stage 2 一起喂，在 finalize job 里算' },
      { label: '自己站点排除', text: '同一个 has_rank_title 开关不区分"自己的站点"和竞品——如果自己的站点也开了排名追踪，会被当成竞品统计进 competitor_tracking_records。2026-08-10 当天先改成 sites.is_own_site 字段手动逐站标"自家/竞品"，几小时后用户反悔改回自动识别（手动标太麻烦），最终维持自动识别：lib/tracking-summary.ts 的 fetchOwnSiteDomains() 读 task_groups.site_domains（分组任务页面的"自己站点"字段，雷达/词库那类页面也用同一个字段），is_own_site 列已从 sites 表删除。竞品成效计算 + 研究中心"竞品成效"tab 的站点列表 + "管理竞品站点"弹窗都用 fetchOwnSiteDomains() 判断，自己的站点不会出现在这三处任何一处' },
      { label: '大环境输入', text: '查这段周期 environment_daily（大盘每日）+ environment_segments_daily（体量档位/内容侧重两个维度的分段数据）全部行，存进 research_reports.environment_input 做历史快照（不依赖以后这两张表的算法是否变化）。2026-08-10 当天曾额外算 environment_stats（取 period_end 当天全站权重+收录快照，用 computeWeightTiers() 把站点分成大/中/小站，代码算出整体+各档位平均PC/移动权重/平均收录数，直接存库不经过AI，页面展示数字卡片），2026-08-11 用户反馈还是想看完整文字说明，改回不算这段结构化数字——environment_stats 字段仍留在表里但新报告不再写入（老报告这列还留着历史值，读接口/页面也不再读取展示，纯遗留不删）；computeEnvironmentStats() 本身没删，已被"站点诊断"功能（见下方同名小节）继续复用' },
      { label: 'Stage 2（一次综合，finalize job）', text: '汇总 research_report_sites 里这份报告的全部站点结果（不分片，一次性查全部）+ 大环境原始数据 + 自己站点成效原始数据（含 contentBreakdown）+ 竞品成效原始数据（含 contentBreakdown）+ 机会缺口列表（见下方同名小节）一次性喂给 Gemini，maxOutputTokens=8192；2026-08-10 起返回 JSON 从4个长字段（300-600字/段）改成 {environmentNote, ownGroupNotes（按分组名为key的对象，每组一段）, competitorNote, conclusion}，除 conclusion 外收紧到100-200字（页面上有对应结构化数字，AI不用复述）；2026-08-11 environmentNote 单独改回 300-600字长文字（大环境不再有数字卡片可看，靠这段文字完整覆盖）；2026-08-12 加第5个字段 opportunityGaps（数组，AI把机会缺口列表聚类成词群，每群给关键词数/总搜索量/优先级high|medium|low/一句建议，列表为空就返回空数组）；同时喂给AI的"各站点AI分析"文本从只拼 summary 改成额外附上每个站点 Stage1 算出来的发力词群摘要（不再只读一句压缩过的话）；conclusion 200-350字（要做自己站点vs竞品的归因对比，篇幅稍长），自己站点和竞品必须分开写；Stage2 本身崩溃会把报告标记 status=failed（不会像单个分片崩溃那样只是不影响别的分片，因为它是最后一段，没有后续步骤能兜底）' },
      { label: '机会缺口（2026-08-12 新增）', text: '回答"市场现在靠哪些词收益，而我方有没有吃到"——用户拿旧prompt问ChatGPT得到的重构建议里，用户明确认定这是最想要的部分。"是否零覆盖"是纯代码判断，不靠AI去对比两份清单（容易漏判/误判），AI只负责聚类/判优先级/给建议。lib/opportunity-gap.ts 的 computeOpportunityGaps()：① lib/competitor-effectiveness.ts 新增 fetchCompetitorWinningKeywords() 查这段周期 competitor_tracking_records 里 effectiveness=有效（不是"追踪中"这种还没确认的）的词，排除自己站点域名，裁到搜索量最高300个；这张表按 site_id+keyword+discovery_date 唯一，追踪窗口内同一个词可能有多条不同 discovery_date 的"有效"记录，按 discovery_date 降序去重只留最新一条（跟"竞品成效展示"tab 的 [id]/tracking 路由踩过的同一个坑，做法照抄）。② lib/tracking-summary.ts 新增 fetchOwnCoveredKeywordSet() 查 site_tracking_records 全部历史（不限周期、不限组——三个月前做过、现在还在排名的词依然算已覆盖，不该因为这期没提交就被判成缺口）的 keyword 集合，这张表持续增长会超3000行硬顶，走 fetchAllRows 分页。③ 竞品词集合减去我方历史覆盖集合（keyword精确字符串比对）就是缺口列表，按搜索量降序喂给 Stage2' },
      { label: '写入表', text: 'research_reports（按 period_type+period_start+period_end 唯一 upsert；own_effectiveness/competitor_effectiveness/environment_input/environment_stats/report_sections 均为 jsonb；旧版遗留的 site_analyses 列不再写入但保留不删，读接口对拆分片之前生成的老报告仍会读这一列做兼容；environment_stats 同理2026-08-11起不再新写入但保留列）/ research_report_sites（每站点一行，按 report_id+site_id 唯一，2026-08-07 新增，随 research_reports 级联删除；2026-08-10 加 pc_weight/mobile_weight/avg_index_count/avg_mobile_ip 4个 numeric 列，老报告这4列是 null，读接口会 fallback 去查一次 weight_history 兜底；2026-08-12 加 momentum_keywords/findings 两个 jsonb 列，老报告这两列是 null，页面对应展示区不渲染）' },
  ...
  },
  {
    ...
    items: [
      ...
      { label: '写入表（2026-08-20 补充，2026-08-26 修正调用频率）', text: 'keyword_signal_rollup——site_id+keyword+type 主键；scripts/crawl.ts 的 runRank()、scripts/crawl-rank.ts、app/api/cron/route.ts 的 rank/rank-title 两个写入点，写完当天 rank_changes/site_keyword_ranks 后调一次 refresh_keyword_signal_rollup(today)。该函数内部按 stat_date=当天 处理全部数据，跟分片无关；GH Actions 分片跑（--group/--total-groups）时改成只有 group=0 那一片调用，其余分片跳过——8/26发现之前每个分片都各调一次，rank-title 8分片+rank约6-8分片一天十几次重复全量重算同一天数据，是CPU/Disk IO被打满、rankWords/streakWords 反复算失败的主因，已修复；app/api/cron/route.ts 走 Vercel 单次触发无分片，照常每次都调' },
    ],
  },
  {
