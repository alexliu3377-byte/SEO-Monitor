...
export const CRAWL_RULES: RuleSection[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { label: 'Stage 1（逐站点，分片内串行）', text: '每个分片内部对分到的 is_enabled=true 站点，用 lib/site-research-summary.ts 的 fetchSiteResearchSummary() 拉这段周期的权重/收录/涨跌/新增关键词/排名成效全量明细（不压缩，月报/年报同样是完整重新读原始数据，不是把周报文字汇总）。2026-08-10 起同时用这份明细代码算出结构化数字（最新一天的 pc_weight/mobile_weight、期间平均收录 avg_index_count、期间平均移动IP avg_mobile_ip），直接存进 research_report_sites 对应4个新列，不经过AI；再用 lib/site-research-prompt.ts 的 buildSiteAnalysisPrompt() 建 prompt（要求AI判断权重上涨是否连续多天而非单日跳动，明确告知页面已单独展示这些数字，AI只需写100-200字点出1-2个最值得注意的点，不用逐项复述），单次 callGeminiJSON 调用；这段时间完全没有任何抓取数据的站点直接跳过，不占AI调用（也不写4个结构化数字列）；每次调用间隔4秒；单站点数据拉取失败或AI调用失败都不中断这个分片，记录失败原因后跳到下一个站点；每个站点跑完立刻 upsert 进 research_report_sites（每站点一行，按 report_id+site_id 唯一——分片并行写不会互相覆盖，这是跟旧版单进程共享一个 JSONB 数组列最大的区别）' },
      ...
      { label: 'Stage 2（一次综合，finalize job）', text: '汇总 research_report_sites 里这份报告的全部站点结果（不分片，一次性查全部）+ 大环境原始数据 + 自己站点成效原始数据（含 contentBreakdown）+ 竞品成效原始数据（含 contentBreakdown）一次性喂给 Gemini，maxOutputTokens=8192；2026-08-10 起返回 JSON 从4个长字段（300-600字/段）改成 {environmentNote, ownGroupNotes（按分组名为key的对象，每组一段）, competitorNote, conclusion}，除 conclusion 外收紧到100-200字（页面上有对应结构化数字，AI不用复述）；2026-08-11 environmentNote 单独改回 300-600字长文字（大环境不再有数字卡片可看，靠这段文字完整覆盖），其余三段维持短字段不变；conclusion 200-350字（要做自己站点vs竞品的归因对比，篇幅稍长），自己站点和竞品必须分开写；Stage2 本身崩溃会把报告标记 status=failed（不会像单个分片崩溃那样只是不影响别的分片，因为它是最后一段，没有后续步骤能兜底）' },
      { label: '写入表', text: 'research_reports（按 period_type+period_start+period_end 唯一 upsert；own_effectiveness/competitor_effectiveness/environment_input/environment_stats/report_sections 均为 jsonb；旧版遗留的 site_analyses 列不再写入但保留不删，读接口对拆分片之前生成的老报告仍会读这一列做兼容；environment_stats 同理2026-08-11起不再新写入但保留列）/ research_report_sites（每站点一行，按 report_id+site_id 唯一，2026-08-07 新增，随 research_reports 级联删除；2026-08-10 加 pc_weight/mobile_weight/avg_index_count/avg_mobile_ip 4个 numeric 列，老报告这4列是 null，读接口会 fallback 去查一次 weight_history 兜底）' },
