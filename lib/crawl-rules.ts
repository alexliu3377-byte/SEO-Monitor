...
export const CRAWL_RULES: RuleSection[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { label: '自动识别', text: 'App Store 可按已知链接或数字 ID 一次导入最多100个应用，自动取得名称与 Bundle ID；抓取时由 Apple 官方接口返回结构化版本资料。普通网页优先读取来源配置的 CSS 选择器，其次读取 meta、JSON-LD、常见“更新日志/版本说明”标题和保守的版本号规则。所有新记录先进入“待审核”，不会直接成为正式最新版本' },
      ...
      { label: '实验边界', text: '当前是 v4.0.0 的线上实验区，不写入正式开发版本记录。App Store 批量能力只监控已知链接或 ID，不遍历整个商店；Google Play 与 TapTap 的批量导入和专用解析仍待后续实验。先验证准确率和导出格式，再决定是否加入定时任务或安装包处理' },
    ],
  },
  {
    key: 'search',
