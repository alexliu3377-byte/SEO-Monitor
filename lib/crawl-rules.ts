...
export const CRAWL_RULES: RuleSection[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { label: '大环境输入', text: '查这段周期 environment_daily（大盘每日）+ environment_segments_daily（体量档位/内容侧重两个维度的分段数据）全部行，存进 research_reports.environment_input 做历史快照（不依赖以后这两张表的算法是否变化）。2026-08-10 起额外算 environment_stats：取 period_end 当天（往前最多找7天找到有 weight_history 数据的那天）全站权重+收录快照，用 lib/weight-tiers.ts 的 computeWeightTiers()（跟大环境分段检测同一套33/66百分位自动分档算法，不用 sites.category 人工分类字段）把站点分成大/中/小站，代码直接算出整体+各档位的平均PC/移动权重、平均收录数，存进 research_reports.environment_stats（jsonb），完全不经过AI，页面直接展示这些数字' },
      { label: 'Stage 2（一次综合，finalize job）', text: '汇总 research_report_sites 里这份报告的全部站点结果（不分片，一次性查全部）+ 大环境结构化数字（environment_stats格式化文本）+ 大环境原始数据 + 自己站点成效原始数据（含 contentBreakdown）+ 竞品成效原始数据（含 contentBreakdown）一次性喂给 Gemini，maxOutputTokens=8192，prompt 明确告知这些结构化数字页面上已经展示、AI不用复述、只需点重点异常因果；2026-08-10 起返回 JSON 从4个长字段（300-600字/段）改成 {environmentNote, ownGroupNotes（按分组名为key的对象，每组一段）, competitorNote, conclusion} ——除 conclusion 外每段收紧到100-200字，conclusion 200-350字（要做自己站点vs竞品的归因对比，篇幅稍长），自己站点和竞品必须分开写；Stage2 本身崩溃会把报告标记 status=failed（不会像单个分片崩溃那样只是不影响别的分片，因为它是最后一段，没有后续步骤能兜底）' },
      { label: '写入表', text: 'research_reports（按 period_type+period_start+period_end 唯一 upsert；own_effectiveness/competitor_effectiveness/environment_input/environment_stats/report_sections 均为 jsonb；旧版遗留的 site_analyses 列不再写入但保留不删，读接口对拆分片之前生成的老报告仍会读这一列做兼容）/ research_report_sites（每站点一行，按 report_id+site_id 唯一，2026-08-07 新增，随 research_reports 级联删除；2026-08-10 加 pc_weight/mobile_weight/avg_index_count/avg_mobile_ip 4个 numeric 列，老报告这4列是 null，读接口会 fallback 去查一次 weight_history 兜底）' },
