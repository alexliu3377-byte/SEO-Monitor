...
export const CRAWL_RULES: RuleSection[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { label: '并行策略', text: '排名段1-5同时并行，段内按页顺序，每页间隔300ms；每页请求带 Referer=上一页URL（第1页 Referer=站点首页），模拟真实"点下一页"的跳转链，而不是每次都用固定 Referer（2026-07-27 加入：实测发现翻页 100% 卡在第1-2页、从未翻到第3页，怀疑反爬专门检查翻页请求的 Referer 链；2026-08 切到纯 fetch 后继续保留这个逻辑）' },
      ...
      { label: '补抓历史日期', text: 'scripts/crawl.ts --step=rank 支持 --date=YYYY-MM-DD 覆盖"今天"（默认取当前 MYT 日期）；爱站排名页本身按日期区分 URL（当天省略日期段，历史日期带日期段），所以可以补抓过去某一天的涨跌词，仅影响 rank 步骤，不影响同一次运行里的其它 step' },
    ],
  },
  {
    key: 'cron_manual',
