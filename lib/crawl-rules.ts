...
export const CRAWL_RULES: RuleSection[] = [
  {
    ...
    items: [
      ...
      { label: '清理', text: '每日关键词步骤结束后由 group0 执行：竞品日收的 raw_keywords（昨日新词）、rank_changes 与 site_keyword_ranks（排名波动）保留40天；每张表每轮最多删除5个过期日期，首次积压分多天消化，避免影响随后任务。keyword_volume、研究报告快照与成效追踪记录不删除。competitor_kw_stats 保留30天（趋势图/工作日周末基线按30天取数）；activity_log 保留7天。' },
  ...
  },
  {
    ...
    items: [
      ...
      { label: '去重规则', text: '原始链接仍按 platform + external_id 唯一；计分与详情再按平台＋规范化标题＋摘要折叠内容相同的页面。不同链接若抽取到相同资料只算一份独立资料；重复采集同一页面只更新指标，不会制造新的趋势证据' },
      { label: '评分口径', text: 'refresh_trend_discovery_terms() 只统计内容不同的独立资料，并以公开时间或首次发现时间判断首次、最近、近24小时、前24小时、跨平台与持续天数。反复抓到同一页面不会延长“持续出现”；百度搜索量、爱站排名和平台搜索建议不直接增加帖子证据分' },
  ...
  {
    key: 'app-update-center',
    title: '应用更新中心（v4.0.0 超管实验）',
    badge: 'GitHub Actions · 第一阶段手动运行',
    items: [
      { label: '触发方式', text: '第一阶段只通过 app-update-crawl.yml 的 workflow_dispatch 手动运行，可选择只抓某个应用或限制本轮来源数量；确认来源稳定后再考虑定时执行。任务使用 GitHub Secrets 中现有的 Supabase 地址与 service role 密钥，不由浏览器直接访问数据库' },
      { label: '采集范围', text: '读取超管登记的公开更新页面，提取版本号、更新日志、发布日期、包大小、公开下载链接及来源页面；App Store 来源使用 Apple Lookup API 专用解析，其他来源继续使用通用网页解析；不登录第三方后台、不绕过验证、不下载或保存 APK/安装包文件' },
      { label: '访问限制', text: '每个来源顺序执行并至少间隔4秒；单页最长等待30秒、响应正文上限5MB、最多跟随3次重定向。每次请求和每一跳重定向都检查 DNS 与目标地址，拒绝本机、私网、保留地址和非标准端口，避免服务端请求伪造' },
      { label: '自动识别', text: 'App Store 可直接从所选地区的免费榜或付费榜自动补全前100名，也可按已知链接或数字 ID 一次导入最多100个应用；Apple 官方公开接口会返回名称、Bundle ID、当前版本、更新日志、发布日期和包大小，后续抓取还会从公开产品页补全同一应用最多50条历史版本。普通网页优先读取来源配置的 CSS 选择器，其次读取 meta、JSON-LD、常见“更新日志/版本说明”标题和保守的版本号规则。所有新记录先进入“待审核”，不会直接成为正式最新版本' },
      { label: '持续扩展', text: '独立 App Store Catalog Discovery 工作流每天运行一次，在 CN/HK/TW/MY/SG/US/JP 商店间轮换；读取免费榜、付费榜并轮换12个搜索词，每个搜索词最多200条。默认一半搜索普通应用，一半搜索游戏；游戏覆盖角色扮演、策略、卡牌、射击、模拟经营、休闲、动作、冒险和生存等类别，并以 Apple Games 分类二次校验，也可手动只扩展游戏。系统按 Bundle ID 与版本去重，持续增加实验目录而不随机枚举 Apple ID' },
      { label: '审核与导出', text: '只有超管可进入应用更新中心；超管可查看来源健康与运行日志、通过或忽略待审核版本，并勾选最多200条记录导出 UTF-8 CSV。只有通过审核的记录才会更新应用当前版本' },
      { label: '去重规则', text: '同一来源按 source_id + normalized_version 唯一；重复抓到相同版本只更新网页提取资料与最后发现时间，不重复建立版本。不同来源的同一版本暂时各自保留，便于比较来源质量' },
      { label: '写入表', text: 'app_update_apps（应用）、app_update_sources（更新来源与健康状态）、app_update_releases（版本及审核状态）、app_update_crawl_runs（每个来源的运行结果）；全部开启RLS并只允许服务端 service role 访问' },
      { label: '实验边界', text: '当前是 v4.0.0 的线上实验区，不写入正式开发版本记录。App Store 榜单补全只覆盖所选地区免费榜和付费榜前100名，不代表完整商店目录；Google Play 与 TapTap 的批量导入和专用解析仍待后续实验。先验证准确率和导出格式，再决定是否加入定时任务或安装包处理' },
    ],
  },
  {
    key: 'search',
...
export const RETENTION = {
  raw_keywords: '40天（竞品日收昨日新词，按 content_date）',
  rank_changes: '40天（普通排名波动，按 stat_date）',
  competitor_kw_stats: '30天（按 stat_date）',
  weight_history: '永久保留',
  index_snapshots: '永久保留',
  keyword_volume: '永久保留',
  site_keyword_ranks: '40天（带标题/URL的M/PC排名波动，按 stat_date）',
  ...
  trend_collection_queries: '永久保留，项目负责人手动替换',
  app_update_apps: '永久保留；停用时改为暂停或归档',
  app_update_sources: '永久保留来源配置与最近健康状态',
  app_update_releases: '永久保留版本、更新日志与审核结果',
  app_update_crawl_runs: '实验阶段永久保留，稳定运行后再评估清理周期',
